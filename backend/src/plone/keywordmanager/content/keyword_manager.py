...
class KeywordManager:
    ...
    @security.protected(utils.MANAGE_KEYWORDS_PERMISSION)
    def change(self, old_keywords, new_keyword, context=None, indexName="Subject"):
        ...
        new_keyword = utils.to_str(new_keyword)
        try:
    ...
    def getKeywordIndexes(self):
        """Gets a list of indexes from the catalog. Uses py to choose the
        ...
        idxs = [
            i.id
            for i in idxs
            if i.meta_type == utils.META_TYPE and i.id not in utils.IGNORE_INDEXES
        ]
        idxs.sort()
        return idxs
    ...
    def fieldNameForIndex(self, indexName):
        ...
        try:
            ...
            raise ValueError(f"Found no index named {indexName}") from IndexError

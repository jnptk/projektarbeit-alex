...
PROJECT_NAME=plone-keywordmanager
STACK_NAME=plone-keywordmanager-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://plone-keywordmanager.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t kitconcept/plone-keywordmanager-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t kitconcept/plone-keywordmanager-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name plone-keywordmanager-frontend-acceptance --link plone-keywordmanager-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d kitconcept/plone-keywordmanager-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name plone-keywordmanager-backend-acceptance -d kitconcept/plone-keywordmanager-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop plone-keywordmanager-frontend-acceptance
	@docker stop plone-keywordmanager-backend-acceptance

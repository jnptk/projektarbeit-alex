...
PROJECT_NAME=projektarbeit
STACK_NAME=projektarbeit-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://projektarbeit.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t collective/projektarbeit-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t collective/projektarbeit-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name projektarbeit-backend-acceptance -d collective/projektarbeit-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name projektarbeit-frontend-acceptance --link projektarbeit-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/projektarbeit-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop projektarbeit-frontend-acceptance
	@docker stop projektarbeit-backend-acceptance
